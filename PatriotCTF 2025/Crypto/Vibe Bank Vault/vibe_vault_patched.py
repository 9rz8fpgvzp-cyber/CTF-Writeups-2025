...
class VibeBankVault:
    def __init__(self):
        ...
        self.secret = "".join(
            random.choices(string.ascii_letters + string.digits, k=140)
        )
    ...
    def _prime_channel(self):
        ...
        token = bcrypt.hashpw(primary[:72], bcrypt.gensalt())
        return token if bcrypt.checkpw(shadow[:72], token) else None
